...
\cite{article:imphlttrigeer:2006}. A discriminação destas
...
O primeiro nível de filtragem opera com granularidade reduzida
para tornar mais rápido o processo de decisão. Então, no L1, as
...
utilizados pelo L1 para definir um possível candidato a elétron:
\begin{enumerate}
...
Neste trabalho, para análise dos algoritmos do L2, foram
...
indicar que não houve isolamento). No segundo conjunto, o corte em
energia utilizou um patamar igual a 15 GeV, e foram realizados
também cortes por isolamento (em energia e hadrônico), produzindo
...
O processo de discriminação de elétrons no segundo-nível de
filtragem \textit{online} do ATLAS, conforme ilustrado na Figura
...
extração de características e teste de hipóteses adotado para o
canal elétron/jato no segundo nível de filtragem do ATLAS. Esse
algoritmo utiliza informações de calorimetria, sendo capaz de
separar objetos eletromagnéticos isolados de jatos hadrô\-nicos,
utilizando parâmetros que estimam a forma dos chuveiros de
deposição de energia. O T2Calo opera de modo semelhante ao
algoritmo de filtragem do L1, porém agora, ao invés de usar as
torres de \textit{trigger}, toda a granularidade dos calorímetros
está disponível.

O primeiro passo do T2Calo é refinar a posição do centro da RoI
...
energia nas camadas hadrônicas por elétrons, o T2Calo opera
...
razão$\frac{E_{3 \times 7}}{E_{7 \times 7}}$ para eventos com
...
medidos no calorímetro (com a construção de anéis a partir do
...
gerado para cada camada. A Tabela \ref{tab_aneis} mostra a
...
algumas camadas, as células não são quadradas (conforme ilustrado
na Figura \ref{fig_ancam}),
...
A Figura \ref{fig_sig3d} mostra, para a segunda camada
eletromagnética, os sinais medidos para um elétron e um jato
típicos e seus respectivos sinais em anéis. A Figura
\ref{fig_sig_anel} mostra sinais em anéis, respectivamente, para
...
elétrons apresenta pouco espalhamento (é contido em uma pequena
...
com pouca concentração espacial (ver Figura \ref{fig_sig_anel}-b).
...
hadrônica representa a maioria dos jatos aprovados como elétrons
...
hipótese) os sinais em anéis precisam ser normalizados. No
...
produzidos. A energia do anel i da camada c ($Ea_{ic}$) é
normalizada através de:
\begin{equation}
...
adotado em todos os resultados a serem mostrados nessa tese.
...
refinados de $E_T$, $\eta$ e $\phi$. Este passo de processamento
...
Ringer} é utilizada pelo T2Calo, então, o restante precisa agora
...
decisiva para o aumento da eficiência de
...
Como os sistemas neurais de classificação são obtidos a partir de
um processo iterativo de ajuste de pesos, um pré-processamento
...
dimensionality})~\cite{book:nnforpr:1995}. A utilização de um
número elevado de entradas para o sistema classificador acaba
dificultando o processo de treinamento. Isso se deve ao fato de
que quanto maior a dimensão dos dados de entrada, maior a
...
Os estudos desenvolvidos durante esse trabalho tiveram o objetivo
de investigar a aplicação da versão não-linear da análise de
componentes independentes
(NLICA)~\cite{book:almeida:2006,article:simas:2007:lnlm} para
extração de características, como uma etapa de pré-processamento
ao \textit{Neural Ringer}.

Considerando que existem diversos algoritmos para a estimação da
NLICA, neste trabalho foram realizados estudos com alguns destes
...
para produzir a decisão. Os resultados obtidos serão mostrados no
Capítulo \ref{cap_resul}.
