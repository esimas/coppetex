...
detector ATLAS. O conhecimento adequado da energia de elétrons e fótons em uma larga faixa (de alguns GeV a 
...
A precisão nas medições dos calorímetros do ATLAS varia com a energia e a posição de interação. A grande quantidade 
de material instalado entre o ponto de colisão e o sistema de calorímetros provoca severas perdas na medição da 
energia de elétrons, conforme mostrado na Figura \ref{cap3_fig_enloss}. Um outro fator que pode provocar medição 
incorreta no calorímetro eletromagnético é o vazamento de energia além da terceira camada eletromagnética. 
Devido a características construtivas do detector, na região de transição entre o barril e a tampa do calorímetro 
(entre $\eta=1.37$ e $\eta=1.52$, conhecido como \textit{crack}), a resolução da energia é bastante degradada.
...
\end{equation}
sendo $s$ um fator global de correção, $c$ uma tendência (ou bias), $w_0$ corrige a energia perdida antes do 
...
são mostrados na Figura \ref{cap3_enres}. Percebe-se que a precisão diminui para baixa energia e próximo ao 
...
Embora estas duas características (concentração em torno do centro da RoI e vazamento hadrônico) 
sejam bastante úteis na identificação de elétrons, uma análise um pouco mais detalhada revela que, 
pelo fato dos jatos apresentarem uma grande flutuação em suas características, o uso apenas destes 
parâmetros pode não produzir o desempenho de discriminação desejado. Na Figura \ref{cap3_anwigmans}, 
considerando a razão $\frac{E_{HAD}}{E_{EM}}$ (num conjunto de eventos simulados), percebe-se que há 
uma razoável superposição entre elétrons e jatos, principalmente em baixa energia. Comportamento 
semelhante é verificado na Figura \ref{cap3_anperfil} onde elétrons e jatos são comparados a partir 
da razão $\frac{E_{P}}{E_{C}}$, sendo $E_{P}$ e $E_{C}$, respectivamente, a energia na periferia e no 
centro da RoI (a célula mais energética e as imediatamente adjacentes a ela foram considerados como 
centro da RoI no cálculo de $E_{C}$, a energia depositada no restante da RoI foi somada para produzir $E_{P}$).
...
uso combinado da física de interesse e do processamento estatístico de sinais em conjunto com classificadores 
capazes de produzir cortes não-lineares no espaço de busca foram propostas como uma alternativa ao T2Calo para
a discriminação elétron/jato no L2. Estes discriminadores, que apresentam elevada eficiência de discriminação,
 serão descritos na próxima seção.

\section{Neural Ringer - Alternativa para Filtragem de Elétrons no L2}
\label{sec_ringer}
Um discriminador alternativo (Neural Ringer) foi proposto em \cite{article:seixas:1996} para o canal 
...
é que, em algumas camadas, as células não são quadradas (por exemplo, a EM1 é composta de 
células que são semelhantes a ranhuras), neste caso, falar em anéis é uma abstração 
(pois ao final do processo descrito acima não existem realmente anéis de células sensoras). 
A Figura \ref{fig_sig3d} mostra os sinais medidos na segunda camada eletromagnética do calorímetro,
...
Para o processo de discriminação (teste de hipóteses) o Neural Ringer utiliza um classificador neural supervisonado 
na arquitetura Perceptron de Múltiplas Camadas (MLP \textit{Multi-Layer Perceptron})~\cite{haykin:nn:2008}. Os 
classificadores neurais são capazes de produzir superfícies não-lineares de separação, aumentando a eficiência 
...
(alta eficiência e processamento rápido) os classificadores neurais são adequados ao ambiente do segundo nível de 
...
de características discriminantes. A seguir estes trabalhos serão descritos brevemente.
...
Componentes Principais (PCA - \textit{Principal Component Analysis}) para compactação. A PCA foi aplicada aos 
sinais em anéis e o classificador neural treinado a partir das componentes principais estimadas. Com a PCA, 
foi alcançado um fator de redução de aproximadamente 4 vezes, mantendo desempenho semelhante ao Neural Ringer 
tradicional. Visando melhorar explorar toda a granularidade e segmentação disponível aos calorímetros do ATLAS, 
...
de Discriminação).  Foi realizado também um estudo referente à aplicação da Análise de Componentes Independentes 
(ICA \textit{Independent Component Analysis}) como uma etapa adicional de pré-processamento, aparecendo logo 
...
identificação de elétrons, na prática, seu comportamento é não-linear, devido a fenômenos como saturação 
...
desenvolvidos durante a elaboração desta tese tiveram o objetivo de investigar a aplicação da versão 
não-linear da análise de componentes independentes (NLICA) para extração de características no segundo 
