...
A física de partículas ou física de altas energias (HEP-\emph{High-Energy Physics)}) busca estudar as 
características fundamentais da matéria. Para isso, ao longo dos anos, vêm sendo contruídos 
experimentos, cada vez mais complexos, que envolvem profissionais de diversas áreas do 
conhecimento como engenharia, computação, 
...
mundo. Na medicina também são diversas as aplicações derivadas dos experimentos de 
física de partículas, como a rádio-terapia e exames tipo a cintilografia e o PET 
...
seleção, onde o ruído de fundo é gradualmente reduzido. 

No detector ATLAS (o maior detector de propósito geral do LHC), o sistema de filtragem é sub-dividido em 
três níveis. Este trabalho propõe um algoritmo alternativo para a filtragem de elétrons 
no segundo nível de \emph{trigger} do ATLAS. Aliando algoritmos de processamento estatístico 
de sinais (baseados no modelo da análise de componentes independentes-ICA), com a exploração 
de toda a segmentação presente nos sinais medidos no sistema de calorímetros, foi possível 
obter um aumento na eficiência de discriminação.

... relembrar que (conforme identificado em outros trabalhos Ringer melhor que T2Calo e 
ICA melhor que ringer)

Foram aplicados três modos distintos para a estimação da análise não-linear de 
componentes independentes (NLICA), o modelo pós não-linear (PNL), mapas auto-organizáveis 
(para estimar um modelo não-linear sem restrições estruturais) e a ICA local. 
...
(... e para os experimentais??). 
O uso do pré-procesamento por mapas auto-organizáveis 
possibilita, além de um discriminador \emph{online} com alta eficiência, a visualização 
dos eventos em gráficos de duas ou três dimensões que podem ser úteis para a análise \emph{offline}. 
...
total de processamento em aproximadamente~30~\%.
