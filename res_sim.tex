...
elétrons que tem energia menor que 4~GeV (e consequentemente pertencem ao primeiro 
\emph{bin} do histograma), nenhum foi classificado corretamente pelo 
T2Calo, $\sim$30~\% foi identificado pelo \emph{Ringer} 
e $\sim$70~\% foi classificado incorretamente por ambos os discriminadores. 
...
Pode-se ver que, 1,03~\% dos jatos são classificados corretamente atravé do 
...
à classes de acerto/erro em questão.
...
e em três faixas em $\eta$ ($\eta\sim 0$, 
...
abordagens de treinamento que podem ser divididas conforme a 
segmentação da informação em segmentada e não-segmentada e conforme 
a utilização de informação supervisionada em não-supervisionada, 
...
Figura~\ref{E10somSegSize} (desta vez para os mapas supervisiondos). 
...
o máximo valor do SP e $\Delta_{SP}$ a variação do $SP$);
 \item Ter o menor número de neurônios entre os mapas que satisfazem às 
restriçoes acima.
\end{itemize}
Pois, desta forma, para o mapa i escolhido garante-se que, o ERM está 
...
Utilizando-se as projeções $\mathbf{u}=\mathbf{W}x$ (onde $\mathbf{W}$ é a matriz 
...
é importante notar que não se garante que, as componentes mais
energéticas (que normalmente são retidas após a PCA) carreguem
...
\hline
    ...
    $\mathbf{P_F}$ \textbf{p/} $\mathbf{P_D=97\%}$ & 1,10$\pm$0,09 & 0,94$\pm$0.105 & 1,21$\pm$0.13 & 3,39$\pm$0.14 & 4,13$\pm$0.18\\
\hline
\end{tabular}
...
elevado de neurônios ocultos, o modelo fica super-dimensionado e não é capaz de estimar adequadamente 
...
as abordagens que consideram a informação supervisionada durantea extração de características 
...
classificadora foi re-treinada retirando-se a informação de
...
queda no SP, e mais ainda, retirando-se ao mesmo tempo as camadas
...
segmentado. Um comparativo da eficiência de cada um dos métodos de
...
justificado pois os classificadores segmentados já resolveram parte 
do problema.}. 
...
Um comparativo da eficiência para diferentes valores de $\eta$ e energia é 
...
o SOM supervisionado não-segmentado e para o PNL o pré-processamento 
...
Pode-se observar que, que o mapeamento não-linear
...
\centering \caption{Comparação dos resultados obtidos para oT2Calo, 
\emph{Neural Ringer} e o discriminador neural operando em conjunto 
com a ICA (linear) para o conjunto E15. São mostrados o
máximo SP($\times$100), as probabilidades de decisão (PD) e falso alarme (PF) para o melhor SP e a
...
A Figura~\ref{figE15ICA} mostra um comparativo entre as eficiências em $\eta$ e 
...
por ICA em conjunto com o classificador neural, produzem uma maior eficiência 
...
\caption[Comparativo da eficiência em energia (esquerda) e $\eta$ (direita) para 
o T2Calo e o discriminador baseado no pré-processamento em ICA com 
compactação por PCD.]{Comparativo da eficiência em energia (esquerda) e $\eta$ 
...
Por outro lado, diferente do observado para o conjunto E10, o 
...
na camada H2 aumentou de 16 para 32. Apenas na camada  E1 o tamanho 
...
maior eficiência do que aquele treinado com toda as sete camadas (de modo 
...
pequeno número de neurônios (no caso 3 neurônios) é suficiente para resolver o problema.
...
sinais tipo E15i o desempenho relativo do SOM foi melhor (em comparação aos demais discriminadores).
Em geral, pode-se verificar que o uso da informação supervisionada produziu aumento 
na eficiência de discriminação em comparação aos mesmos algoritmos treinados de modo não 
supervisionado.
...
desempenho equivalente (ligeiramente inferior) ao \emph{Neural Ringer} com redução do tempo 
...
SOM Sup. Seg.& 95,22 $\pm$ 0,11 & 96,87 $\pm$ 0,41 & 6,41 $\pm$ 0,31 & 6,61 $\pm$ 0,27\\ \hline
 Local  & 95,85 $\pm$ 0,21 & 96,02 $\pm$ 0,29& 4,31 $\pm$ 0,43& 6,54 $\pm$ 0,45\\ \hline
 PNL+PCA& 94,55 $\pm$ 0,17 & 96,13 $\pm$ 0,32 & 7,01 $\pm$ 0,37 & 8,42 $\pm$ 0,33\\ \hline
 PNL+PCD& 95,20 $\pm$ 0,13 & 96,88 $\pm$ 0,25& 6,46 $\pm$ 0,41& 6,67 $\pm$ 0,31\\ \hline
PNL Mod.& 95,54 $\pm$ 0,19  & 97,57 $\pm$ 0,43& 6,47 $\pm$ 0,29& 5,98 $\pm$ 0,35\\ \hline
Rede Comb.& 96,01 $\pm$ 0,20 & 96,47 $\pm$ 0,37 & 4,45 $\pm$ 0,37 & 5,42 $\pm$ 0,30\\ \hline
...
A partir dos resultados obtidos com os conjunto simulados foi possível chegar a algumas 
conclusões:
\begin{itemize}
...
interconexões entre os módulos do calorímetro) e faixas de energia (principalmente as baixas energias) 
