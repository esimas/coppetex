...
Com o pré-processamento por ICA e ICA Local, foi possível melhorar
...
Ainda na Figura~\ref{figcosmicBarC}, pode-se observar uma comparação
...
\end{minipage}
\begin{minipage}[b]{0.98\linewidth}
  \centering \footnotesize{Comparação de rejeição}
 \end{minipage}
...
têm, em média, um perfil que se assemelha ao perfil médio de
elétrons.
...
\end{minipage}
\caption{Comparação de desempenho em energia e $\eta$ de diferentes discriminadores na
...
Conforme discutido no Capítulo~\ref{cap_metod}, os sinais experimentais disponíveis até o
...
Deste modo, aplicando-se diretamente aos sinais experimentais à
...
a energia é mais baixa;
 \item Maior influência de fenômenos inerentes dos sinais experimentais como ruído
...
maior separação entre as classes. Ainda na mesma Figura, podem-se visualizar as
...
onde os valores do SP se mantêm próximos ao máximo. Assim, ajustando-se o patamar para -0,70, é possível obter
...
\caption{Variação do SP e das probabilidades de detecção e falso alarme com o patamar de decisão.} \label{fig_exp_spPDpf}
\end{figure}
...
pseudo-rapidez ($\eta$). Embora os dois discriminadores tenham sido ajustados
...
\end{minipage}
\hfill \caption{Comparação de desempenho entre o \emph{Neural Ringer} e os
dois cortes do T2Calo.}\label{fig_compRingerT2}
\end{figure}

Considerando agora o corte T2EM3\_\emph{Medium}, de modo semelhante ao
verificado no critério T2EM3\_\emph{Loose},
para uma mesma probabilidade de detecção ($\sim$96~\%), o \emph{Neural Ringer}
reduz significativamente
o falso alarme de modo aproximadamente uniforme em energia.
Analisando as distribuições em
...
obtidos para os sinais simulados. Neste contexto, visando a uma melhor caracterização
das assinaturas que o \emph{Ringer} classifica incorretamente, foram selecionados
os eventos onde a saída do classificador neural era muito próxima ao valor-alvo
...
modelos/abordagens de treinamento a seguir:
\begin{itemize}
 \item ICA (linear e segmentado) com compactação por PCD;
 \item ICA Local (não-segmentado);
...
\caption{Distribuição das assinaturas de cada classe nos agrupamentos.} \label{figExpLocalClus}
\end{figure}
...
de corte dos classificadores locais que maximizam o SP.
...
não-segmentado, o melhor compromisso entre os dois parâmetros foi
...
problema era baixo, pois a melhor eficiência de discriminação foi obtida quando as funções
não-lineares foram estimadas por redes neurais de apenas dois neurônios ocultos. A
compactação a partir do modelo PNL modificado produziu o número de componentes conforme
listado na Tabela~\ref{tabexp1CompPNL}. Pode-se observar que a redução foi semelhante
à obtida a partir da PCD.


\begin{table}[b!]
\centering \caption{Número de componentes estimados a partir da
compactação por PCD e pelo modelo PNL modificado.}\vspace{0.2cm} \footnotesize
\begin{tabular}{c | c c c c c c c | c}
    \hline
     \textbf{Layer} & \textbf{PS} & \textbf{E1} & \textbf{E2} & \textbf{E3} & \textbf{H0} & \textbf{H1} & \textbf{H0} & \textbf{Total} \\
\hline
    \textbf{Rings} & 8 & 64 & 8 & 8 & 4 & 4 & 4 & 100 \\
      \textbf{PCD} & 6 & 5 & 5 & 6 & 3 & 3 & 2 & 30 \\
      \textbf{PNL Mod.} & 6 & 7 & 5 & 5 & 3 & 3 & 2 & 31 \\
    \hline
\end{tabular}
\label{tabexp1CompPNL}
\end{table}
...
\begin{tabular}{c|c|c|c|c|c|c}
  ...
  SOM Sup. & 86,26 $\pm$ 0,20 & 85,34 $\pm$ 0,29 & 12,81 $\pm$ 0,11 & 26,81 $\pm$ 0,17 & 40,65 $\pm$ 0,25 & 55,89 $\pm$ 0,27\\ \hline
   \bf PNL Mod.&\bf 86,06 $\pm$ 0,18 &\bf 86,10 $\pm$ 0,15 &\bf 13,98 $\pm$ 0,12 &\bf 25,60 $\pm$ 0,21 &\bf 39,01 $\pm$ 0,22 &\bf 56,01 $\pm$ 0,26\\ \hline
...
Em sequência, a rede combinadora foi treinada utilizado a
informação dos classificadores especialistas. O processo de
treinamento foi reiniciado retirando-se, por camada, a informação
...
\end{minipage}
\hfill \caption{Comparativo de desempenho entre o T2Calo, \emph{Neural Ringer} e
a rede combinadora.}\label{figCombClasPhis}
\end{figure}
...
\end{minipage}
\hfill \caption{Comparação de desempenho entre diferentes discriminadores
...
e os erros inerentes à identificação \emph{offline},
...
\begin{enumerate}
    ...
    dados utilizados pelo T2Calo $\rightarrow$ (0,4927$\pm$0,0787)~ms;
    \item \textbf{Pré-processamento} (T2Calo) - as variáveis de decisão do
    T2Calo são calculadas $\rightarrow$ (0,1408$\pm$0,0148)~ms;
    \item \textbf{Seleção de região} (\emph{Ringer})  - são selecionados os
    dados utilizados pelo \textit{Neural Ringer} $\rightarrow$ (0,4375$\pm$0,0996)~ms;
    \item \textbf{Anelamento} - construção dos anéis $\rightarrow$ (0,0986$\pm$0,0165)~ms;
    \item \textbf{Normalização} - o vetor com os anéis é normalizado $\rightarrow$ (0,0026$\pm$0,0015)~ms;
    \item \textbf{Classificação Neural} - a rede neural produz a decisão
    de aceitação/rejeição do evento $\rightarrow$ (0,0387$\pm$0,0018)~ms.
\end{enumerate}
...
As rotinas para operação da ICA Local e do modelo PNL e do SOM foram
implementadas no sistema de filtragem do ATLAS, e seu custo
computacional avaliado foi a partir da propagação de um conjunto de
eventos através das cadeias de processamento. Nesta
...
\begin{enumerate}
    \item \textbf{Seleção do \emph{cluster}} - as assinaturas que chegam
    são divididas nos dois \emph{clusters} a partir do cálculo da
    distância (euclidiana) para o centro de cada \emph{cluster};
    \item \textbf{ICA$+$Rede Neural} - dentro de cada \emph{cluster}, as
...
contribui pouco para o aumento do custo computacional, resultando
...
realizando-se os processos de extração de características e classificação de modo segmentado,
...
(anelamento) e normalização para cada camada do calorímetro. Pode-se
observar que a primeira camada
...
anelamento e normalização gasto para cada camada do calorímetro (direita).}\label{figTemposSeg}
\end{figure}

Considerando o uso dos classificadores segmentados, foi mostrado
que é possível obter eficiência de discriminação equivalente à do
\emph{Neural Ringer} (em alguns casos superior), mesmo sem
utilizar a informação de algumas das camadas. A maior redução que
ainda preserva bom desempenho para todos os conjuntos de dados
...
\begin{table}[hb!]
\centering \caption{Comparação entre o \emph{Neural Ringer} e a rede combinadora que
