...
teoricamente e a identificação de novos fenômenos. O experimento LHC
\cite{article:LHC:2008} é o maior acelerador de partículas já
...
1950, quando foram descobertas centenas de novas partículas.
...
fraca se destacam. A intensidade relativa dos quatro tipos de
...
\begin{tabular}{cc }
  ...
  \hline
\end{tabular}
\caption{Intensidade relativa dos diversos tipos de interação.}
\label{tab_forcas}
\end{table}
...
A massa (m) das partículas é usualmente expressada em função da
...
com os experimentos em operação antes do LHC, sua massa deve ser
...
precisam ser respondidas. É um desejo antigo dos físicos, desde
...
dezenas de paises \cite{Homepage:CERN}.
...
mostrados na Figura pois estão em locais próximos respectivamente ao
...
atual\-mente e funciona  no Fermilab.
...
a multiplicidade de partículas e faixas de ener\-gia, em conjunto
...
Caminhando do eixo central para as extre\-mi\-dades, em seqüência
estão os calorímetros, que medem a energia depositada pelas
partículas. Na interação com as células do calorímetro são
produzidos chuveiros de partículas secundárias. Num último estágio
está o detector de múons, onde câmaras imersas em um campo magnético
medem a curvatura da partícula.
...
Calorímetros \cite{book:wigmans:2000} são uma importante classe de
...
calorímetros podem apresentar resposta muito rápida, da ordem de 10
a 100 ns \cite{book:martin:2006}, e por isso é utilizado no sistema
...
para seu perfil de deposição de energia.
...
com o calorímetro eletromagnético, mas somente são completamente
absorvidas nas camadas hadrônicas (mais externas).

O sistema de calorímetros do detector ATLAS
\cite{article:ATLAS:2008} é sub-dividido em 7 camadas, sendo 4
...
eletromagnético.} \label{calo_cell}
\end{figure}
...
Devido a suas características construtivas, a resolução dos
calorímetros $R_E$ é descrita por \cite{book:wigmans:2000}:
\begin{equation}\label{res_calo}
    R_E \propto \frac{1}{\sqrt{E}}
\end{equation}
onde E é a energia incidente por partícula. Foi calculada
experimentalmente em~\cite{atlascalo:2002} a resolução esperada para
o calorímetro do ATLAS. Os valores encontrados foram $0,1/\sqrt{E}$
...
tornar mais rápida a tomada de decisão, a granularidade dos
subsistemas do detector é reduzida \cite{TDR:ATLAS:1998}.
Considerando o sistema de calorímetros, que possui mais de 180.000
células detectoras, o LVL1 utiliza apenas 7000 torres de soma
analógicas (as torres são obtidas somando-se a energia de células
dentro de regiões de 0,1$\times$0,1 em $\Delta\eta \times
\Delta\phi$). Até a tomada de decisão de nível 1 toda a informação
do detector é armazenada em memórias tipo \textit{pipeline}.
Quando um evento é aceito, as informações referentes são
descarregadas para uso pelo nível 2 de filtragem. As informações
dos eventos não aceitos são descartadas neste ponto.
...
\begin{enumerate}
  ...
  são somadas duas a duas e utiliza-se o maior valor encontrado, que deve exceder um patamar de energia EM;
...
taxa de eventos de 75 kHz (podendo chegar até 100 kHz) para 3,5
kHz, tendo um tempo de latência de aproximadamente 40 ms para
tomar a decisão \cite{TDR:ATLAS:2003}. O EF precisa diminuir a
taxa de eventos de 3,5 kHz para 200 Hz. Os eventos que forem
aceitos pelos três níveis de filtragem serão armazenados em mídia
permanente para futura análise \textit{offline}. O tempo para a
tomada de decisão no filtro de eventos é de alguns segundos. O HLT
é implementado em \textit{software} e opera em um conjunto de PCs
em paralelo. Embora existam algoritmos desenvolvidos pela
colaboração do ATLAS para a filtragem de alto nível (nos diversos
canais de interesse), pesquisas continuam sendo conduzidas com o
objetivo de propor rotinas de filtragem alternativas que forneçam
maior eficiência de discriminação da física de interesse e, ao
mesmo tempo, maior rejeição do ruído de fundo.
...
\begin{itemize}
  ...
  \item robustez em relação à luminosidade, ruído de medição, imperfeições de alinhamento e calibração.
...
com o melhor conhecimento do detector, após os testes e o início
de operação. Percebe-se, então, que o sistema de \textit{trigger}
de alto nível demanda esforços no sentido de propor, testar e
...
a discriminação elétron/jato no LVL2. Exemplos podem ser encontrados
em \cite{article:anjos:2006} e \cite{article:herman:2006}.

Como resultado destes trabalhos, foi implementado na plataforma de
software do ATLAS o discriminador Neural-Ringer, que utiliza um
...
processamento que, embora seja maior que o do T2Calo, atende aos
