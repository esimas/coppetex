...
visualizar o diagrama de um mapa auto-organizável bidimensional.
...
à ICA (\emph{Independent Component Analysis} ou Análise de Componentes Independentes) 
por conseguirem extrair informações ocultas dos sinais de
forma não supervisionada \cite{article:foo:2006}. Uma apro\-xi\-mação
dos componentes independentes não-lineares pode ser obtida
...
do neurônio j pode ser definido por: $\mathbf{w}_j=[w_{1j},w_{2j},...,w_{kj}]^T$.
A atualização do vetor de pesos é feita através da equação:
\begin{equation}\label{pesos_som}
    \mathbf{w}_j(n+1)=\mathbf{w}_j(n)+\eta (n) h_{ij}(n)(\mathbf{x}(n)-\mathbf{w}_j(n)),
\end{equation}
\\
sendo $\eta (n)$ a taxa de aprendizagem. A função de vizinhança é escolhida
de modo que tem seu valor máximo em $j$ e decresce à medida que se afasta.
...
topológico, isto é, padrões de entrada semelhantes são mapeados para regiões
...
espaço de saída;
  ...
  \item a matriz de pesos sinápticos pode ser definida por:
\begin{equation}
...
aquele que minimiza $i(\mathbf{x})=\| \mathbf{x}_A-\mathbf{w}_j\|$.
...
Auto-Organizáveis, pois aqui os componentes independentes também
...
adaptação dos parâmetros do modelo; nesse sentido, o processo é
semelhante ao utilizado pela abordagem da inferência Bayesiana,
que será mostrada com mais detalhes a seguir. Não foram
encontradas muitas aplicações do GTM na estimação do modelo 
não-linear da análise de componentes independentes (NLICA).

\subsection{Quantização Vetorial por Aprendizado}

A quantização vetorial (VQ-\textit{Vector Quantization}) é uma
técnica de codificação em que um espaço de entrada é mapeado em um
...
Em um problema de classificação, pode-se empregar a quantização vetorial por aprendizado (\textit{LVQ-Learning Vector
...
neural tipo Percéptron de Múltiplas Camadas (MLP-\emph{Multi-Layer Perceptron}) é conectada às saídas do SOM (ver Figura
...
No processamento de sinais multidimensionais, é comum a utilização de técnicas de processamento de sinais
que visam à redução da dimensionalidade do problema. O objetivo é mapear os sinais N-dimensionais observados em 
em um espaço de dimensão K$<$N. Entre as técnicas de compactação, a análise de componentes principais 
...
transformação linear tal que os sinais projetados sejam
...
equivalentes. Limitações computacionais na extração dos componentes
...
dos componentes principais, através de redes neurais
...
reconstrução do sinal projetando os componentes numa base
...
encontrado, o produto $\mathbf{w}_i^Tx$ dará os componentes
...
processo correspondente ao componente $i$.

A transformação por PCA é ótima no sentido de representação do sinal
nos primeiros componentes, mas não há garantia de que a compactação
...
Entretanto, em problemas de classificação nos quais a dimensão da entrada
é excessivamente grande o pré-processamento por PCA reduz o custo
computacional e consequentemente o tempo de processamento.
...
No modelo básico da ICA (ver equações (\ref{ica_eq}) e (\ref{icamatrix})), assume-se que
...
a extração dos componentes independentes é obtido do teorema do
...
A \textbf{curtose} (kurt) é o cumulante de quarta ordem, e para uma
...
\begin{equation}\label{entro1}
    H(Y)=-\sum_i P(Y=a_i)\text{log} P(Y=a_i),
\end{equation}
\\
...
transformação inversa da ICA, conforme equação \ref{ica_inv}, pode
...
descorrelação não-linear, como o \textit{Hérault-Jutten}
...
e $g(.)$ visando-se a obter boas aproximações dos componentes
independentes. O algoritmo \textit{Hérault-Jutten}, por exemplo,
aconselha o uso de $f(y)=y^3$ e $g(y)=\text{arctg}(y)$, já o
...
Em geral, os algoritmos de extração dos componentes independentes
...
para facilitar o processo de extração dos componentes independentes.

\subsection{Principais Algoritmos para ICA}

Diversos algoritmos vêm sendo propostos para a extração dos
componentes independentes. Essas rotinas diferem basicamente no
princípio teórico no qual fundamentam a obtenção dos componentes
...
na extração dos componentes.

Um estudo comparativo entre diversos métodos de estimação dos
...
algoritmo FastICA para estimação de um componente independente é
...
\begin{enumerate}
  ...
  \parallel$;
  \item Se o algoritmo não tiver convergido, volte para o passo 2.
\end{enumerate}
...
há informação a respeito da estatística dos componentes
...
quando os componentes independentes são super-gaussianas e o
$g_3(.)$ deve ser utilizada para estimar componentes sub-gaussianas.

Para estimar mais de um componente independente pode-se utilizar
...
\begin{split}
\text{cum}(x_i,x_j,x_k,x_l)=E\{x_i,x_j,x_k,x_l\}
...
Considerando isso, os métodos Tensoriais de ICA propõem a
...
decomposição em autovalores de matrizes de quarta-ordem. O
algoritmo JADE propõem um método aproximado para a diagonalização de
$\mathbf{T_4}$, tornando-se mais leve computacionalmente.
...
automatrizes são descritas por:
\begin{equation}\label{eq_jade01}
    \mathbf{M}=\mathbf{w}_m\mathbf{w}_m^T
\end{equation}
...
$F(\mathbf{M})$ para qualquer $\mathbf{M}$ (tal que \linebreak
$\mathbf{Q}=\mathbf{W}F(\mathbf{M}_i)\mathbf{W}^T$ é uma matriz diagonal).

A função custo do método JADE busca a diagonalização de
$\mathbf{Q}$ pela maximização da soma dos elementos de sua
diagonal. As matrizes $\mathbf{M}_i$ utilizadas são as
auto-matrizes do tensor cumulante dos dados, pois assim tem-se um
conjunto de N matrizes que contêm toda a informação relevante a
respeito dos cumulantes.
...
algoritmo para a estimação dos componentes independentes no modelo
pós não-linear.

\subsection{Algoritmo Taleb-Jutten para o Modelo Pós Não-Linear (PNL)}
...
estatística dos componentes independentes estimados através do
...
\emph{Kullback-Lieber} é utilizada para encontrar as regras de aprendizado
