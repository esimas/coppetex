...
A maioria  dos fenômenos físicos que são estudados atualmente em
...
dedicado\footnote{O \textit{hardware} utilizado no detector é 
composto de equipamentos disponíveis comercialmente e de sistemas 
desenvolvidos especificamente para o ATLAS pelos membros da colaboração.}. 
...
Assim, este estágio pode ser implementado através de
...
em subsistemas responsáveis pela detecção de classes específicas
...
física de altas energias desde o início da década de 1990, com o
...
utilizados classificadores neurais na arquitetura Percéptron de
...
assinaturas não têm estrutura temporal, representando as medições
instantâneas dos diversos subdetectores. Finalmente, a
...
40 vezes melhor. Uma rede neural tipo Percéptron de
...
interesse), sendo que, em alguns momentos, o \textit{trigger} neural
operou sozinho.
...
ruído de fundo composto por chuveiros induzidos por hádrons 
...
deposição de energia nos calorímetros foram utilizadas para
...
discriminador \textit{Neural Ringer}, que utiliza informação
especialista da física de interesse para pré-processamento dos
dados brutos do calorímetro, produzindo anéis concêntricos de
...
extremamente alta se comparada às demais partículas elementares
...
Percéptron de Múltiplas Camadas foi utilizada para
...
neurais, o seu uso está longe de ser uma unanimidade na
...
experimento precisou se convencer de que as redes não eram uma
...
$\sim$1,5~MByte de informação, são produzidos no detector
...
disponíveis informações (que podem ser descarregadas pelos
...
condições de operação do detector), implicando uma redução da
...
Considerando o filtro de múons, cada um dos subdetectores é
...
assinatura analisada satisfaz um critério de aceitação
 de algum dos subdetectores (calorímetros, RPC ou TGC), então ela é aceita pelo L1 e enviada para
...
sistema de filtragem do L1 é implementado utilizando \emph{hardware}
...
da informação do detector são necessários para produzir a decisão do
...
de seleção do segundo nível, que utilizam a plena granularidade dos
...
\begin{itemize}
  ...
  alarme);
  \item robustez em relação a luminosidade, ruído de medição, imperfeições de ali\-nha\-men\-to e calibração.
\end{itemize}
...
aceito pelo EF, todas as informações referentes a ele são
...
\cite{tese:andre:2006,tese:torres:2010} que é um discriminador
alternativo para o canal elétron/jato com desempenho
superior ao do T2Calo (discriminador oficial do ATLAS) e tempo de
...
A colaboração do ATLAS desenvolveu um conjunto de \emph{softwares} e
ferramentas de controle (conhecidos como Athena)
\cite{TP:athena:2001}, que permitem aos membros da colaboração,
independentemente de sua localização geográfica, acesso e análise dos
...
sua necessidade específica;
 ...
 \item Existe facilidade para reuso de código, poupando tempo no desenvolvimento de rotinas mais complexas.
\end{itemize}
