...
destacam quando o pro\-blema tratado tem características
não lineares e o ambiente de busca é multimodal (existem diversos
...
métodos numéricos exigem que a função objetivo $f(\mathbf{x})$ seja
...
populacional (isto é, explorando outras regiões do espaço de busca).
Algumas características interessantes dos AG's são listadas a
seguir:
\begin{enumerate}
...
entretanto, a maioria dos métodos assim denominados têm em comum
as seguintes características: população composta de cromossomos,
seleção de acordo com a função de adequabilidade (ou função-objetivo), recombinação e
...
\begin{itemize}
    ...
    códigos de \emph{gray}, e até codificação decimal. Na codificação binária
    ...
    \item \textbf{População} - Conjunto de cromossomos gerado inicialmente de forma aleatória. A po\-pulação é
     atualizada a cada nova i\-te\-ração do algoritmo, e espera-se que a cada
    ...
    \item \textbf{Recombinação} (\textit{Crossover}) - Promove a troca de informação
     ...
     trocado. Existem vários modos de proceder a esta troca de bits,
     e cada um dá origem a um tipo diferente de recombinação. A seguir
     ...
     em cromossomos binários:
        ...
        \begin{itemize}
            ...
            aleatório e são trocados os bits destes pontos. É gerado um cromossomo aleatório,
        ...
        \end{itemize}
Na figura \ref{crossover} são ilustrados os processos de recombinação de ponto único e uniforme.
...
específicos, $p_{rec}$ (probabilidade de recombinação) e
$p_{mut}$ (probabilidade de mutação), res\-pectivamente. Estes
parâmetros estabelecem a taxa de ocorrência dos processos.
Exemplo: para $p_{rec}=0.7$, a cada geração, 70\% dos indivíduos
são selecionados para participar da recombinação; com
...
\end{equation}
\\
onde os coeficientes $a$ e $b$ são escolhidos previamente.
...
para obtenção dos parâmetros $(x_{i})$, e com estes calculam-se os
...
partes do espaço de busca, enquanto valores de $p_{mut}$ muito altos podem
...
do \textit{single-point crossover}, proposto por Goldberg, em que é
escolhido um ponto e são trocados os bits que estão depois deste
