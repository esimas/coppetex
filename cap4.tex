...
que visam a redução da dimensionalidade do problema...
...
Conforme proposto em \cite{}, para um problema de classificação, o objetivo da PCD é obter uma projeção linear 
...
\cite{} ilusta a aplicação da NLICA num problema de classificação de sinais de eletroencefalograma (EEG). O objetivo é a 
...
independência e um algoritmo genético \cite{} buscou sua minimização. Múltiplos classificadores lineares (cada um 
...
A NLICA foi utilizada no trabalho \cite{} visando à extração de características de séries temporais de ações para a 
