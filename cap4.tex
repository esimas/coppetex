...
Considerando um problema de discriminação onde existem apenas duas classes possíveis, os PCD podem ser estimados a partir de uma rede neural (de arquitetura MLP - \textit{Multi-Layer Perceptron})~\cite{haykin:nn:2008} com uma camada oculta e um neurônio de saída, treinada para obter máxima discriminação entre as classes. Conforme indicado na Figura~\ref{fig_pcd}, uma rede neural com um neurônio na camada oculta é capaz de estimar o primeiro PCD, que é obtido a partir da projeção das entradas na direção dos pesos sinápticos do neurônio oculto:
\begin{equation}
 z_1=[b_{1,1},b_{1,2},...,b_{1,N}]\times[x_1,...,x_N]^T + b_{0,1},
\end{equation}
onde $b_{0,1}$ é o \textit{bias} do neurônio. Adicionando-se mais neurônios ocultos consegue-se estimar os demais PCD. No processo de estimação dos PCD, estatística de ordem elevada é acessada a partir da utilização de funções de ativação não-lineares.
