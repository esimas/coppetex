...
estimação da NLICA no contexto deste trabalho. A seguir, será
...
\footnote{Sinais multi-dimensionais são, em geral, produzidos por
sistemas de medição com múltiplos sensores, mas também podem
surgir a partir da aplicação de transformações (como a
transformada de Fourier ou Wavelet) a sinais uni-dimensionais.}
...
multi-dimensionais, pode-se mencionar a Análise de Componentes
...
saída são mutuamente independentes estatisticamente. A ICA vêm
...
o conhecimento de toda a estatística dos dados, sendo assim muito
...
verificar se existem restrições quanto ao aumento do custo
...
\begin{itemize}
    ...
    \item As pdf dos componentes independentes são limitadas a faixas de valores conhecidos.
...
é estimado pelo algoritmo). Entre os modelos com restrições
...
possíveis sinal estimado é igual ao número de neurônios utilizados
para representá-lo).
...
o objetivos eram, respectivamente, separação de imagens
...
multi-dimensionais apresenta grande variação em suas
...
ou seja, em subconjuntos onde os elementos tem características
semelhantes.
...
dados de alta dimensão pode ser separado em sub-conjuntos (onde os
...
eletrocardiograma, da sobre-vida de pacientes que sofreram ataque
...
estatístico multi-variável vêm sendo aplicados com sucesso em
...
\rightarrow$quark+antiquark$\rightarrow$hadrons. O objetivo é
...
métodos de seleção de varáveis como o teste
...
ao algoritmo tradicional utilizado para o problema, porém
...
análise de dados multi-variados em experimentos de física atômica
e nuclear. A aplicação de ICA proporcionou redução do ruído de
fundo, permitindo melhor visualização do sinal de interesse.

ICA também foi aplicado com sucesso para separação de sinais em
...
analisar as informações (mapas de radiação) gerados pelo satélite,
...
foi obtida uma relação sinal/ruído (S/R) da ordem de 20~dB,
...
possível separar os espectros infra-vermelhos provenientes de
...
rádio-telescópios. A ICA mostrou-se bastante eficiente para
...
garantia que a transformação por ICA/NLICA seja útil nesta tarefa.
