...
características discriminantes se tornem mais acessíveis. As
características extraídas são utilizadas como entrada para os
classificadores propriamente ditos (classificadores neurais
supervisionados na arquitetura MLP). Nesta Seção, serão descritos
os sistemas de filtragem e o procedimento de treinamento
utilizados.

Os métodos de extração de características utilizados neste
trabalho operam sobre os sinais dos calorímetros do ATLAS
formatados em anéis (conforme descrito no Capítulo
\ref{cap_atlasEtrig}). O propósito é aumentar a eficiência de
discriminação de elétrons do \textit{Neural Ringer} (discriminador
de elétrons alternativo do segundo nível de filtragem online do
ATLAS).

Considerando que cada assinatura é descrita por 100 anéis, gerados
em sete camadas do calorímetro, e que, cada camada apresenta
diferentes características físicas (quantidade e tipo de sensores)

A estimação das características discriminantes dos anéis pode ser
realizada de duas formas distintas conforme descrito a seguir.
...
\cite{book:vantrees1:2001}, que mostra como as probabilidades de
detecção e falso alarme (respectivamente PD e PF) variam com o
patamar de decisão, e o índice SP \cite{article:anjos:2006}, que é
definido por:
...
\subsection{Características Globais}
...
\section{Classificadores Neurais Supervisionados}
...
\subsection{Especificações dos Classificadores Neurais Utilizados}
...
tipo tangente hiperbólica. Em trabalhos anteriores
\cite{tese:andre:2006,tese:torres:2010}, bons resultados de
...
\subsection{Classificador Global}
...
\subsection{Classificadores Segmentados}
...
pode ser realizada através de estratégias distintas
