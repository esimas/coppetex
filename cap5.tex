...
Havendo desbalanceamento entre a quantidade de amostra nos conjuntos 
...
apresentando uma baixa relação sinal-ruído (e consequentemente não
...
\begin{itemize}
 ...
 neste caso foi utilizado o mapa auto-organizável - SOM;
...
índice soma-produto (SP), conforme descritos a seguir:
\begin{description}
...
treinamento é realizado através de um algoritmo gradiente que
...
utilizam métodos de busca globais, como propostos
...
observar também alguma relação entre as camadas eletromagnéticas e
...
justificado pois, devido à seleção mais exigente no primeiro
nível, deixando passar apenas uma classe de assinaturas de jatos
que apresenta características mais semelhantes aos elétrons.
...
acontecimentos que se suscederam à origem do universo. O LHC tem
...
baixa, foram gravados todas as assinaturas aprovadas pelo primeiro
...
parâmetros podem ser considerados individualmente ou combinadas em
...
\textit{offline} (estes eventos tem maior probabilidade de serem
...
de software do ATLAS utilizava, no momento da aquisição dos dados,
...
foram semelhantes ao da primeira \emph{run} (todas as assinaturas
...
maiores energias (o que pode ser justificado pela luminosidade
3$\times$ maior). Os perfis em $\eta$ são bastante semelhantes
